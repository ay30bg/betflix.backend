...
exports.getBetHistory = async (req, res) => {
  try {
    ...
    const validBets = bets.filter(
      (bet) => bet.type && bet.value && Number.isFinite(bet.amount) && bet.period
    );
    res.json(validBets);
  } catch (err) {
    console.error(`[${new Date().toISOString()}] Error in getBetHistory:`, err.message, err.stack);
...
exports.placeBet = async (req, res) => {
  try {
    const { type, value, amount, clientSeed, period } = req.body;
    console.log(`[${new Date().toISOString()}] Received bet:`, { userId: req.user.id, type, value, amount, clientSeed, period });
    ...
    if (!period || !/^round-\d+$/.test(period)) {
      console.error(`[${new Date().toISOString()}] Invalid or missing period: ${period}`);
    ...
    const roundStart = Math.floor(now / roundDuration) * roundDuration;
    const roundEnd = roundStart + roundDuration;
    const expectedPeriod = `round-${roundStart}`;

    if (period !== expectedPeriod) {
      console.error(`[${new Date().toISOString()}] Period mismatch: received=${period}, expected=${expectedPeriod}`);
      return res.status(400).json({ error: 'Period does not match current round' });
    }
