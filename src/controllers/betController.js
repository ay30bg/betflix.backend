...
exports.getBetResult = async (req, res) => {
  try {
    ...
    let round = await Round.findOne({ period });
    if (!round || !round.resultNumber || !round.resultColor) {
      console.error('Round or result not found:', { period });
      return res.status(404).json({ error: 'Round result not set' });
    }
...
// Set round outcome (admin)
exports.setRoundOutcome = async (req, res) => {
  const { period } = req.params;
  const { resultNumber, resultColor } = req.body;

  console.log('setRoundOutcome called:', { period, resultNumber, resultColor, adminId: req.admin?.id });

  if (!Number.isInteger(resultNumber) || resultNumber < 0 || resultNumber > 9) {
    console.error('Validation error: Invalid resultNumber', { resultNumber });
    return res.status(400).json({ error: 'Result number must be an integer between 0 and 9' });
  }
  if (!['Green', 'Red'].includes(resultColor)) {
    console.error('Validation error: Invalid resultColor', { resultColor });
    return res.status(400).json({ error: 'Result color must be Green or Red' });
  }
  if (!/^round-\d+$/.test(period)) {
    console.error('Validation error: Invalid period format', { period });
    return res.status(400).json({ error: 'Invalid period format' });
  }
  ...
  try {
    ...
    if (round.resultNumber !== undefined || round.resultColor) {
      console.error('Result already set:', { period, resultNumber: round.resultNumber, resultColor: round.resultColor });
      return res.status(400).json({ error: 'Result already set for this round' });
    }

    round.resultNumber = resultNumber;
    round.resultColor = resultColor;
    await round.save();
    console.log('Round updated:', { period, resultNumber, resultColor });

    const bets = await Bet.find({ period });
    console.log('Found bets:', { count: bets.length, period });
    for (const bet of bets) {
      ...
      if (payout > 0) {
        ...
        if (user) {
          const newBalance = Math.max(user.balance + payout, 0);
          await User.findByIdAndUpdate(bet.userId, { balance: newBalance, updatedAt: Date.now() });
          console.log('User balance updated:', { userId: bet.userId, newBalance });
        } else {
          console.warn('User not found for bet:', { userId: bet.userId, betId: bet._id });
        }
      }
    }

    res.json({ message: 'Round outcome set successfully', result: { resultNumber, resultColor } });
  } catch (err) {
