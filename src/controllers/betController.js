...
exports.getBetHistory = async (req, res) => {
  try {
    ...
    const bets = await Bet.find({
      userId: req.user.id,
      ...(includePending === 'true' ? {} : { status: { $in: ['won', 'lost'] } }),
    })
      .sort({ createdAt: -1 })
      .limit(10);
    res.json(bets);
...
exports.cleanupStaleBets = async (req, res) => {
  try {
    const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
    const result = await Bet.updateMany(
      { status: 'pending', createdAt: { $lt: fiveMinutesAgo } },
      { status: 'lost', payout: 0, updatedAt: new Date() }
    );
    console.log(`[${new Date().toISOString()}] Cleaned up stale pending bets:`, result);
    res.json(result);
  } catch (err) {
    console.error(`[${new Date().toISOString()}] Error in cleanupStaleBets:`, err.message, err.stack);
    res.status(500).json({ error: 'Server error', details: err.message });
  }
};
...
exports.setRoundOutcome = async (req, res) => {
  try {
    ...
    // Finalize all pending bets for this round
    await Bet.updateMany(
      ...
      },
      {
        ...
        result: resultColor,
        won: true,
        payout: (bet) => bet.amount * PAYOUT_MULTIPLIERS.color,
        updatedAt: new Date(),
      }
    );
    await Bet.updateMany(
      ...
      },
      {
        ...
        won: true,
        payout: (bet) => bet.amount * PAYOUT_MULTIPLIERS.number,
        updatedAt: new Date(),
      }
    );
    await Bet.updateMany(
      {
        ...
        status: 'pending',
        $nor: [
          { type: 'color', value: resultColor },
          { type: 'number', value: resultNumber.toString() },
        ],
      },
      {
        status: 'lost',
        result: resultColor || resultNumber.toString(),
        won: false,
        payout: 0,
        updatedAt: new Date(),
      }
