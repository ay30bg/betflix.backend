...
// New admin endpoint to set round outcome
exports.setRoundOutcome = async (req, res) => {
  ...
  if (!['Green', 'Red'].includes(resultColor)) {
    return res.status(400).json({ error: 'Invalid result color' });
  }
  ...
  try {
    ...
    if (round.resultNumber || round.resultColor) {
    ...
    res.json({ message: 'Round outcome set', result: { resultNumber, resultColor } });
  } catch (err) {
    console.error('Error setting round outcome:', err.message, err.stack);
    res.status(500).json({ error: 'Server error', details: err.message });
  }
};
