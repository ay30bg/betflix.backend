...
exports.setRoundOutcome = async (req, res) => {
  try {
    ...
    const gracePeriod = 10000; // 10 seconds
    if (round.expiresAt < new Date() - gracePeriod) {
      return res.status(400).json({ error: 'Cannot set outcome for expired round' });
    }
    ...
    await processBetsForRound(period, resultNumber, resultColor);

    // Emit Socket.IO event
    if (req.app.get('io')) req.app.get('io').emit('roundOutcome', { period, resultNumber, resultColor });

    res.json({ period, resultNumber, resultColor });
  } catch (err) {
    console.error('Error in setRoundOutcome:', err.message, err.stack);
    res.status(500).json({ error: 'Server error', details: err.message });
  }
...
async function processBetsForRound(period, resultNumber, resultColor) {
  try {
    const bets = await Bet.find({ period });
    for (const bet of bets) {
      if (bet.result && bet.won !== undefined) continue;
      ...
      if (payout > 0) {
        ...
        if (user) {
          const newBalance = Math.max(user.balance + payout, 0);
          await User.findByIdAndUpdate(bet.userId, { balance: newBalance, updatedAt: new Date() });
        }
      }
    }
  } catch (err) {
    console.error('Error processing bets for round:', err.message, err.stack);
    throw err;
  }
}
