...
const initiateCryptoDeposit = async (req, res) => {
  ...
  if (normalizedCrypto === 'USDT' && !['BEP20', 'ARBITRUM', 'TON'].includes(network)) {
    return res.status(400).json({ error: 'Invalid or missing USDT network (must be BEP20, ARBITRUM, or TON)' });
  }
  ...
  try {
    let payCurrency = normalizedCrypto.toLowerCase();
    if (normalizedCrypto === 'USDT') {
      if (network === 'ARBITRUM') payCurrency = 'usdtarb'; // Verify with NOWPayments API
      else if (network === 'BEP20') payCurrency = 'usdtbsc';
      else if (network === 'TON') payCurrency = 'usdtton';
    }
    console.log('Mapped payCurrency:', payCurrency);
...
const initiateCryptoWithdrawal = async (req, res) => {
  ...
  // Stricter validation for Arbitrum (Ethereum-compatible) addresses
  if (normalizedCrypto === 'USDT' && network === 'ARBITRUM' && !/^0x[a-fA-F0-9]{40}$/.test(walletAddress)) {
    return res.status(400).json({ error: 'Invalid Arbitrum wallet address (must be 42 characters starting with 0x)' });
  }
  // General wallet address validation for other networks
  if (!['USDT'].includes(normalizedCrypto) || (normalizedCrypto === 'USDT' && network !== 'ARBITRUM')) {
    if (!/^[a-zA-Z0-9]{26,48}$/.test(walletAddress)) {
      return res.status(400).json({ error: 'Invalid wallet address (26-48 characters)' });
    }
  }
  if (!withdrawalPassword) {
