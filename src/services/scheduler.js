...
async function cleanupExpiredRounds() {
  console.log(`[${new Date().toISOString()}] Starting cleanup of expired rounds, MongoDB connected: ${mongoose.connection.readyState === 1}`);
  while (mongoose.connection.readyState !== 1) {
    console.log(`[${new Date().toISOString()}] Waiting for MongoDB connection...`);
    await new Promise(resolve => setTimeout(resolve, 1000));
  }
  ...
  try {
    const now = new Date();
    const gracePeriod = 10000; // 10 seconds, matching getBetResult
    ...
    console.error(`[${new Date().toISOString()}] Error cleaning up expired rounds:`, {
      message: err.message,
      stack: err.stack,
      mongoConnected: mongoose.connection.readyState === 1,
    });
...
function startScheduler() {
  ...
  // Schedule daily cleanup of expired rounds at 1 AM (Africa/Lagos)
  cron.schedule('0 1 * * *', async () => {
    try {
      console.log(`[${new Date().toISOString()}] Daily cleanup of expired rounds triggered`);
      await cleanupExpiredRounds();
    } catch (err) {
      console.error(`[${new Date().toISOString()}] Error in daily cleanup:`, {
        message: err.message,
        stack: err.stack,
      });
    }
...
module.exports = { startScheduler, generateRound, checkSchedulerHealth, testScheduler, preGenerateRoundsFor24Hours, cleanupExpiredRounds };
