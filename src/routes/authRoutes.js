...
const express = require('express');
const router = express.Router();
const { login, logout, signup, forgotPassword, resetPassword, adminSignup, adminLogin } = require('../controllers/authController');
const rateLimit = require('express-rate-limit');
...
// Rate limiter for admin signup/login: 10 requests per 15 minutes per IP
const adminAuthLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 10,
  message: { error: 'Too many admin authentication requests, please try again later.' },
});
